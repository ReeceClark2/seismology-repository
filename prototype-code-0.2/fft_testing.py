...
d = (a1 * np.sin(2 * np.pi * f1 * t) * np.e ** (-k1 * t) +
     a2 * np.sin(2 * np.pi * f2 * t) * np.e ** (-k2 * t) + e)
...
# 2. Calculate Hilbert Transform (Analytic Signal & Envelope)
analytic_signal = hilbert(d)
amplitude_envelope = np.abs(analytic_signal)

# Run BATS model
model = BATS(t, d, min_f, max_f, f_init=[2.5, 5.0], mode="local", signals=2, burn_in=100, typical_set=200, dense_mass=True, f_bw=[100, 100], k_bw=[100, 100], std=1)
model.launch()
...
ax1.plot(t, amplitude_envelope, color='tab:red', linewidth=2, label='Hilbert Envelope')
