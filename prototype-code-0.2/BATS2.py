...
def _parallel_worker(kwargs):
    """Helper function to run BATS instances in isolated processes."""
    model = BATS(**kwargs)
    model.launch()
...
class BATS():
    def __init__(self,
        ...
        processes: Int specifying how many parallel processes to split the frequency range across.
        """
        
        # Store initial keyword arguments for clean parallel duplication
        self._original_kwargs = {
            't': t, 'd': d, 'f_init': f_init, 'k_init': k_init,
            'k_type': k_type, 'mode': mode, 'min_signals': min_signals,
            'max_signals': max_signals, 'signals': signals,
            'sampler': sampler, 'acceptance': acceptance,
            'burn_in': burn_in, 'typical_set': typical_set,
            'dense_mass': dense_mass, 'boundaries': boundaries,
            'f_bw': f_bw, 'k_bw': k_bw, 'std': std
        }
        ...
        # If multiple processes are specified, bypass deep initialization and handle in launch()
        if self.processes > 1:
            self.mode = mode
            return
        ...
        # In __init__, replace the f_init block with this:
        if f_init:
            ...
            if len(f_init) < signals or len(f_init) < max_signals:
                ...
                # Fill remaining slots safely
                needed = self.signals - len(f_init)
                available = min(needed, len(f))
                self.f_init[len(f_init):len(f_init)+available] = f[:available]
                
                # If still short, fill with evenly spaced frequencies in the range
                if available < needed:
                    fill_start = len(f_init) + available
                    self.f_init[fill_start:] = np.linspace(self.min_f, self.max_f, needed - available + 2)[1:-1]
        else:   
            f, p = self._get_fft()
            f, _ = self._get_peaks(f, abs(p), limit=self.signals)
            self.f_init[:len(f)] = f
            
            # If not enough peaks found in this chunk, pad with linspace to avoid identical 0.0s
            if len(f) < self.signals:
                needed = self.signals - len(f)
                self.f_init[len(f):] = np.linspace(self.min_f, self.max_f, needed + 2)[1:-1]
    ...
    def _get_ks(self):
        '''
        Helper function finding the initial decay rates for the initial frequencies.
        '''
        ks = np.zeros(len(self.f_init))
        
        for ind, f in enumerate(self.f_init):
            # Skip if frequency is somehow 0 to avoid filter blowups
            if f <= 0:
                ks[ind] = 1e-4
                continue
            ...
            try:
                ...
                if len(peak_t) > 1:
                    ...
                    ks[ind] = max(-1 * res.slope, 1e-6) # Enforce positive decay
                else:
                    ks[ind] = 1e-4 # Fallback if no peaks survive
            except ValueError:
                # Catch filter design errors if min_f/max_f are outside Nyquist
                ks[ind] = 1e-4
    ...
    def nuts_sampler_unbounded(self):
        # Prevent exact zeros which cause singular matrices and violate constraints.positive
        f_init_safe = jnp.asarray(self.f_init)
        k_init_safe = jnp.maximum(jnp.asarray(self.k_init), 1e-6)

        init_dict = {"fs": f_init_safe, "ks": k_init_safe}
        ...
        # Save to CSV using the helper
        self._save_mass_matrices(inv_mass_matrix, mass_sqrt, len(self.f_init))

        pe = mcmc.get_extra_fields()["potential_energy"]
        best_index = jnp.argmin(pe)

        fs_out = samples["fs"][best_index]
        ks_out = samples["ks"][best_index]

        return fs_out, ks_out
    ...
    def nuts_sampler_bounded(self, fs, ks, f_bw, k_bw):
        ...
        # Save to CSV using the helper
        self._save_mass_matrices(inv_mass_matrix, mass_sqrt, len(fs))
    ...
    def _save_mass_matrices(self, inv_mass_matrix, mass_sqrt, signals):
        """Helper to save NUTS mass matrices to CSV with proper chunk naming."""
        prefix = f"{signals}_model"
        if self._chunk_idx is not None:
            prefix = f"{signals}_model_chunk{self._chunk_idx}_{self.min_f:.4f}_{self.max_f:.4f}"

        # Extract arrays from NumPyro's PyTree (handles both dicts and flat arrays)
        flat_inv_mass = jax.tree_util.tree_flatten(inv_mass_matrix)[0]
        flat_mass_sqrt = jax.tree_util.tree_flatten(mass_sqrt)[0]
        
        # Convert all pieces to float and concatenate into a single 1D array
        inv_mass_array = np.concatenate([np.atleast_1d(np.asarray(x, dtype=float)) for x in flat_inv_mass])
        mass_sqrt_array = np.concatenate([np.atleast_1d(np.asarray(x, dtype=float)) for x in flat_mass_sqrt])

        np.savetxt(f"{prefix}_inv_mass_matrix.csv", inv_mass_array, delimiter=",")
        np.savetxt(f"{prefix}_mass_matrix_sqrt.csv", mass_sqrt_array, delimiter=",")
    ...
    def run_local_model(self, signals):
        ...
        _, SNR, variance, uncertainties = self.get_model_statistics(fs, ks)
        
        f_unc = uncertainties[:len(fs)]
        k_unc = uncertainties[len(fs):]

        header_metadata = (
            f"SNR: {SNR:.4f} | Noise Variance: {variance:.6e}\n"
            f"frequency,decay_rate,f_uncertainty,k_uncertainty"
        )

        sort_indices = np.argsort(fs)
        fs = fs[sort_indices]
        ks = ks[sort_indices]
        f_unc = f_unc[sort_indices]
        k_unc = k_unc[sort_indices]

        # Force the final stack into a pure numeric float array
        output_data = np.column_stack((fs, ks, f_unc, k_unc)).astype(float)

        filename = f"{signals}_model.csv"
        if self._chunk_idx is not None:
            filename = f"{signals}_model_chunk{self._chunk_idx}_{self.min_f:.4f}_{self.max_f:.4f}.csv"
    ...
    def launch(self):
        if self.processes > 1:
            # Parallel execution across split frequency ranges
            freq_bins = np.linspace(self.min_f, self.max_f, self.processes + 1)
            chunks = [(freq_bins[i], freq_bins[i+1]) for i in range(self.processes)]
            
            kwargs_list = []
            for i, (c_min, c_max) in enumerate(chunks):
                kw = self._original_kwargs.copy()
                kw['min_f'] = c_min
                kw['max_f'] = c_max
                kw['processes'] = 1  # Ensure workers do not recursively split
                kw['_chunk_idx'] = i + 1
                
                # Unset init guesses when chunking so each range searches freely for peaks
                kw['f_init'] = None
                kw['k_init'] = None
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=self.processes) as executor:
                futures = [executor.submit(_parallel_worker, kw) for kw in kwargs_list]
...
if __name__ == "__main__":
    ...
    model = BATS(t, 
                 ...
                 boundaries=True, 
                 std=5,
                 processes=2) # Example of the new keyword in action
    model.launch()