...
class BATS():
    ...
    def hamiltonian_monte_carlo(self, 
            ...
            freq_bandwidth=0.000001,  # Absolute bandwidth for frequencies (+/-)
            decay_bandwidth=0.000003): # Absolute bandwidth for decay rates (+/-)
        ...
        freq_lower = np.array(guess_frequencies) - freq_bandwidth
        freq_upper = np.array(guess_frequencies) + freq_bandwidth
        decay_lower = np.array(guess_decay_rates) - decay_bandwidth
        decay_upper = np.array(guess_decay_rates) + decay_bandwidth

        lower_bounds = np.concatenate([freq_lower, decay_lower])
        upper_bounds = np.concatenate([freq_upper, decay_upper])
        ...
        for iteration in tqdm(range(B), desc="Burn-in Samples"):
            ...
            for _ in range(L):
                ...
                under = q < lower_bounds
                over = q > upper_bounds
                q = np.where(under, 2 * lower_bounds - q, q)
                q = np.where(over, 2 * upper_bounds - q, q)
                p = np.where(under | over, -p, p)
        ...
        with open(f'BATS_model_results_{r}.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            # Create columns that define the exact state of every single data point
            writer.writerow(['iteration', 'function', 'frequency', 'decay_rate', 'probability'])
            
            for history_id, history in enumerate(typical_set_history):
                f_list, alpha_list, prob = history
                
                # Zip f and alpha together and write a row for each pair
                for step_index, (f, alpha) in enumerate(zip(f_list, alpha_list)):
                    writer.writerow([history_id, step_index, f, alpha, prob])
